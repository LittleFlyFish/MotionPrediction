...
class AEDecoder(VAE.DecoderBase):
    ...
    def forward(self, decoder_hidden, dec_in, decoder_category, reuse=False, trainable=True, is_training=True):
        with tf.variable_scope(self.name_scope) as vs:
            ...
            h0 = tcl.fully_connected(y, 400, scope="fc3", activation_fn=lrelu, weights_regularizer=tcl.l2_regularizer(self.re_term))

            h0 = tcl.dropout(h0, 0.5, is_training=is_training)

            h0 = tcl.fully_connected(h0, 200, scope="fc5", activation_fn=lrelu, weights_regularizer=tcl.l2_regularizer(self.re_term))
...
class Discriminator(VAE.DiscriminatorBase):
    ...
    #
    #         local_h0 = tcl.fully_connected(dec_in_enc, self.nfilters * 8, scope="fc2", weights_regularizer=tcl.l2_regularizer(self.re_term))
    #
